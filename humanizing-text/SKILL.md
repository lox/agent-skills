...
This skill reviews or rewrites text that already exists. When drafting new text, use `writing-plainly` instead.
...
2. **Review construction first** - For multi-paragraph text, read `reference/construction-patterns.md` and check patterns 25-38. Make only structural edits you can justify from the text before polishing wording; do not restructure a piece to satisfy the checklist.
...
6. **Restore voice** - Bring back real personality or experience that the draft flattened. Never invent it.
...
Never inject synthetic humanity: fixes must bring out something true. Ask when only the author knows; use an explicit placeholder only when the user wants a draft with gaps marked.
