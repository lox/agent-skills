...
Drive a review-fix-validation loop for a current PR or branch. Use `general-code-reviewing` as the review engine; unlike a normal review, act on evidenced ship-risk and simplicity findings and re-check fixes until confidence is high.
...
- Use the caller's required output format when there is one, including for the loop summary.
...
- Keep review passes read-only. Make edits only after synthesizing findings and deciding which fixes are evidenced and in scope.
...
- Do not claim high confidence while validation is failing or real findings remain unresolved.
...
2. Triage: convert findings into actions. Fix critical, high, and medium findings that are evidenced, reachable, and in scope. Fix low findings only when they are cheap or clearly quality-relevant. Defer out-of-scope, pre-existing, or speculative concerns with evidence.
...
6. Repeat the fix and targeted-recheck round once when real findings remain and another focused fix is justified.
7. Run a second full review only when the fixes changed the structure or behavior of the diff, the targeted recheck exposed cross-cutting risk, or real findings remain after the first pass's rounds.
...
- the latest applicable full review and targeted recheck have no findings left after synthesis;
...
Stop with `needs-attention` when fixable findings remain but the user-set budget, review budget, or loop limit prevents another useful iteration. Report exactly which findings remain actionable.
...
Apply stop states in this order: `blocked`, then `needs-attention`, then `ready`. A partial approval cannot override a blocker, and reaching the two-pass or per-pass round cap with findings remaining is `needs-attention`.
...
Write the final response per `writing-plainly`. Start with `Status: ready | needs-attention | blocked`, then a short paragraph for each of these that has content: what the review passes found and what was fixed, the validation commands and their results, residual risks or deferred items, and the branch, commit, and PR details when something was committed or pushed. Skip anything empty.
